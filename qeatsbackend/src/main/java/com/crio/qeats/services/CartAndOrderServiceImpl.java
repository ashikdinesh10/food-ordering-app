...
public class CartAndOrderServiceImpl implements CartAndOrderService {
  ...
  @Override
  public GetOrdersResponse updateStatusOfPlacedOrders(
      String restaurantId, String orderId, String status) {
    Order order = orderRepositoryService.getOrderById(orderId);
  ...
  @Override
  public CartModifiedResponse clearCart(String cartId) {
    try {
      Cart updatedCart = cartRepositoryService.removeCart(cartId);
      response.setCart(updatedCart);
      response.setCartResponseType(0);
    } catch (CartNotFoundException e) {
      System.out.println("Cart Not Found " + e);
